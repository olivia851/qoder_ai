...
check_scaffold_compliance() {
    ...
    if [ -f "req/evaluation-system/pom.xml" ]; then
        echo "🔍 检查后端脚手架配置..."
        # 检查Spring Boot版本
        if grep -q "spring-boot-starter-parent.*2\.9\.0" "req/evaluation-system/pom.xml"; then
            echo "✅ 后端脚手架Spring Boot版本正确"
        else
            echo "❌ 后端脚手架Spring Boot版本不匹配"
            return 1
        fi
    fi
